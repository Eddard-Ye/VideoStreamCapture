...
if sys.platform == "win32":
    for _sdk_dir in _SDK_RUNTIME_DIRS:
        if os.path.isdir(_sdk_dir):
            if hasattr(os, "add_dll_directory"):
                os.add_dll_directory(_sdk_dir)
            os.environ["PATH"] = _sdk_dir + os.pathsep + os.environ.get("PATH", "")
    MvCamCtrldll = ctypes.WinDLL("MvCameraControl.dll")
else:
    _runenv = os.getenv("MVCAM_COMMON_RUNENV")
    if not _runenv:
        raise RuntimeError("MVCAM_COMMON_RUNENV is not set for Linux MVS SDK.")
    MvCamCtrldll = ctypes.cdll.LoadLibrary(os.path.join(_runenv, "64", "libMvCameraControl.so"))
